...
const PluginErMaterialsPane = (ctx: IPublicModelPluginContext, options: any) => {
  const {hideLeftArea} = options || {}
  return {
    name: "ModelActionsPane",
    init() {
      const title = '数据模型行为'
      ctx.skeleton.add({
        name: "modelActionPane",
        area: "leftArea",
        type: "PanelDock",
      ...
      ctx.skeleton.showPanel('modelActionPane');
    },
  };
};

PluginErMaterialsPane.pluginName = 'plugin-er-materials-pane';
PluginErMaterialsPane.meta = {
...
export default PluginErMaterialsPane;