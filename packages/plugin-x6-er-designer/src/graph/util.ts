...
export function getShortBothPort(source: any, target: any) {
  ...
  var sourcePorts = getNodePorts(source)
  var targetPorts = getNodePorts(target)
  var closestSourcePort = null
  var closestTargetPort = null
  var minDistance = Infinity
  sourcePorts.forEach((sourcePort: any) => {
    targetPorts.forEach((targetPort: any) => {
      var distance = Math.sqrt(Math.pow(targetPort.x - sourcePort.x, 2) + Math.pow(targetPort.y - sourcePort.y, 2))
